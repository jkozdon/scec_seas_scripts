...
if ($Page->is_success)
{
  ...
  foreach (split(/\n/,$Page->content))
  {
    ...
    my @datatypes = ();
    if(index($station,"fltst") != -1)
    {
      @datatypes = ( "slip", "slip_rate", "shear_stress", "state");
    }
    else
    {
      print "unkown station type $station\n";
      next;
    }
    ...
    foreach(@datatypes)
    {
      ...
      {
        $_ =~ /".*(http:\/\/scecdata.usc.edu\/cvws\/cgi-bin\/seas.cgi\/cvws.gif[^"]*).*"/ || next;
        my ($url) = ($1);
        $url =~ s/amp;//g;
